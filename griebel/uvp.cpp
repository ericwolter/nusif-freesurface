...
//  Purpose:
//
//    COMP_TEMP computes the new temperature.
//
//  Author:
//
    ...
    for (i = 1; i <= imax; i++)
        for (j = 1; j <= jmax - 1; j++)
        {
                    ...
                    ((FLAG[i][j + 1] & C_F) && (FLAG[i][j + 1] < C_E)))
            {
                ...
                // cout << "V[i][j]: " << V[i][j] << endl;
                // cout << "delt: " << delt << endl;
                // cout << "LAPLV: " << LAPLV << endl;
                // cout << "DUVDX: " << DUVDX << endl;
                // cout << "DV2DY: " << DV2DY << endl;
...
//  Purpose:
//
//    COMP_RHS computes the right hand side of the pressure equation.
//
//  Author:
//
//    Michael Griebel, Thomas Dornseifer, Tilman Neunhoeffer
...
//  Purpose:
//
//    POISSON carries out an SOR iteration on the Poisson equation for the pressure.
//
//  Author:
//
//    Michael Griebel, Thomas Dornseifer, Tilman Neunhoeffer
...
//  Purpose:
//
//    COMP_DELT computes the time stepsize satisfying the CFL stability criteria.
//
//  Discussion:
//
//    The routine also sets the flag "write" if some data has to be written
//    into a file.
//
//  Author:
//
...
//
{
    ...
    if (tau >= 1.0e-10)
    {
        ...
        else if (deltv < deltRePr) *delt = deltv;
        else           *delt = deltRePr;
        //
