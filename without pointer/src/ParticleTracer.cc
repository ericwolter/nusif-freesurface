...
void ParticleTracer::addRectangle(int x1, int y1, int x2, int y2, int type)
{
    ...
    for (int i = minX; i < maxX; ++i)
    {
        for (int j = minY; j < maxY; ++j)
        {
            ...
            fillCell(i + 1, j + 1, grid_.ppc(), type);
        }
    }
}
...
void ParticleTracer::addCircle(int x, int y, int r, int type)
{
    ...
    for (int i = -r; i < r; ++i)
    {
        ...
        for (int j = -h; j < h; ++j)
        {
            fillCell(x + i, y + j, grid_.ppc(), type);
        }
    }
}
...
void ParticleTracer::advanceParticles(real const dt)
{
    ...
    for (unsigned int i = 0; i < particles_.size(); ++i)
    {
        ...
        real u = interpolateU(particles_[i].x(), particles_[i].y());
        real v = interpolateV(particles_[i].x(), particles_[i].y());

        // std::cout << "TRACER oldPosX,Y: " << p->x() << ", " << p->y() << std::endl;
        particles_[i].setX(particles_[i].x() + dt * u);
        particles_[i].setY(particles_[i].y() + dt * v);
        // std::cout << "TRACER newPosX,Y: " << p->x() << ", " << p->y() << std::endl;

        int newCellX = particles_[i].getCellX(grid_.dx());
        int newCellY = particles_[i].getCellY(grid_.dy());
