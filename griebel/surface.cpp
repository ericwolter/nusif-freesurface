...
//
{
    ...
    for (int i = minX; i < maxX; ++i)
    {
        for (int j = minY; j < maxY; ++j)
        {
            ...
            //  Set the particles.
            //
            double cellX = (x - 1) * delx;
            double cellY = (y - 1) * dely;
            std::cout << "TRACER cellX,Y: " << cellX << ", " << cellY << std::endl;

            int particlesPerSide = (int)(sqrt(ppc));
            std::cout << "TRACER particlesPerSide: " << particlesPerSide << std::endl;

            double deltaX = delx / (particlesPerSide);
            double deltaY = dely / (particlesPerSide);
            std::cout << "TRACER deltaX,Y: " << deltaX << ", " << deltaY << std::endl;
            for (int q = 1; q <= particlesPerSide; ++q)
            {
